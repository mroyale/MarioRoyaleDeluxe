...
/* Returns the [x, y] location of the top left corner of a tile in the sprite sheet. Location is in pixels.. but 24!! */
util.sprite.getSprite24 = function(texture, index) {
  var w = texture.width;
  var h = texture.height;
  var i = index * Display.TEXRES_24;

  var y = parseInt(Math.floor(i / w) * Display.TEXRES_24);
  var x = i % 2;
...
/* Returns if the target number is an even number, or if it's not. If it is, returns true, if it isn't, returns false. */
util.isEven = function(int) {
  return int % 2 === 0;
};
...
/* Fades the page IN, used for main menu */
function fadeIn() {
...
/* Fades the page OUT, used for main menu */
function fadeOut() {
  var elem = document.getElementById("body");
...
/* Does the same thing as fadeOut however with a call back that is called at the end */
